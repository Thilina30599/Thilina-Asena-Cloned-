...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp... Please Wait.')}`);
    ...
    WhatsAsenaCN.on('credentials-updated', async () => {
        ...
        console.log(chalk.bgGreen('ThilinaBot' + afwhasena));
        await new Promise(r => setTimeout(r, 500));
        let EVA_ACTİON = config.LANG == 'TR' || config.LANG == 'AZ' ? '*ThilinaBot Olarak Çalışıyor!* 🤖\n\n_Bu modun amacı botu tam fonksiyonel bir yapay zeka sohbet aracına çevirmektir._\n_Normal moda dönmek için_ *.fulleva off* _komutunu kullanabilirsiniz._\n\n*ThilinaBot Kullandığın İçin Teşekkürler 💌*\n    *- Eva*' : '*ThilinaBot Working as a Chatbot! 🤖*\n\n_The purpose of this mod is to turn the bot into a fully functional AI chatbot._\n_You can use the_ *.fulleva off* _command to return to normal mode._\n\n*Thanks For Using ThilinaBot 💌*\n    *- Eva*'
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                if (error.message.includes('URL')) {
                                    return await WhatsAsenaCN.sendMessage(WhatsAsenaCN.user.jid, '*⚕️ HATA ÇÖZÜMLEME [ThilinaBot] ⚕️*' + 
                                        '\n========== ```Hata Okundu!``` ==========' +
                                ...
                                if (error.message.includes('URL')) {
                                    return await WhatsAsenaCN.sendMessage(WhatsAsenaCN.user.jid, '*⚕️ ERROR ANALYSIS [ThilinaBot] ⚕️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
