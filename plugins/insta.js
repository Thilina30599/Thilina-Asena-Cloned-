...
Asena.addCommand({ pattern: 'vinsta ?(.*)', fromMe: false, desc: sd }, async (message, match) => {
    ...
    await axios.get(`https://docs-jojo.herokuapp.com/api/insta?url=${userName}`).then(async (response) => {
        ...
        if (resource.url.is_video[0]) {
            await message.sendMessage(Buffer.from(profileBuffer.data), MessageType.video, { caption: 'Made by *🤖𝓣𝓱𝓲𝓵𝓲𝓷𝓪 𝓑𝓸𝓽🤖*' })
        }
        else if (!resource.url.is_video[0]) {
            await message.sendMessage(Buffer.from(profileBuffer.data), MessageType.image, { caption: 'Made by *🤖𝓣𝓱𝓲𝓵𝓲𝓷𝓪 𝓑𝓸𝓽🤖*' })
        }
    }).catch(async (err) => {
